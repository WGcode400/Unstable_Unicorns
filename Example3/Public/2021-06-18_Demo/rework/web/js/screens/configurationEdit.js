...
)
{
  ...
  return class ConfigurationEdit
  {
    ...
    //-------------------------------------------------------------------------
    constructor( accutechAPI, system, map, view, saveCallback )
    {
      $.when( template ).done
      (
        function()
        {
          ...
          const configurationEditAreaTemplate = $( "#configurationEditAreaTemplate" ).html()
          $( "main" ).prepend( configurationEditAreaTemplate )
          ...
          const tabClasses = {}
          tabClasses.assets         = new AssetsTab(      tabs.get( "assetsTab"      ), accutechAPI, system, map, view, saveCallback )
          tabClasses.clients        = new ClientsTab(     tabs.get( "clientsTab"     ), accutechAPI, system, map, view, saveCallback )
          tabClasses.placement      = new PlacementTab(   tabs.get( "placementTab"   ), accutechAPI, system, map, view, saveCallback )
          tabClasses.views          = new ViewsTab(       tabs.get( "viewsTab"       ), accutechAPI, system, map, view, saveCallback )
          tabClasses.receivers      = new ReceiversTab(   tabs.get( "receiversTab"   ), accutechAPI, system, map, view, saveCallback )
          tabClasses.systemTab      = new SystemTab(      tabs.get( "systemTab"      ), accutechAPI, system, map, view, saveCallback )
          tabClasses.areasTab       = new AreasTab(       tabs.get( "areasTab"       ), accutechAPI, system, map, view, saveCallback )
          tabClasses.controllersTab = new ControllersTab( tabs.get( "controllersTab" ), accutechAPI, system, map, view, saveCallback )
