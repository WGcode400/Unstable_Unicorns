      ...
      if self.handshaked is False:
         ...
         if not data:
            ...
            if b'\r\n\r\n' in self.headerbuffer:
               ...
               except Exception as e:  # $$$DEBUG - Limit exceptions excepted.
                  hStr = FAILED_HANDSHAKE_STR
                  self._sendBuffer(hStr.encode('ascii'), True)
                  self.client.close()
                  raise WebSocketException('handshake failed: %s', str(e))
   ...
   def _sendBuffer(self, buff, send_all = False):
      ...
      while tosend > 0:
         try:
            ...
            if sent == 0:
               raise WebSocketException('socket connection broken')
...
class SimpleWebSocketServer(object):
   ...
   def serveonce(self):
      ...
      for ready in wList:
         ...
         except Exception as n:  # $$$DEBUG - Limit exceptions excepted.
            self._handleClose(client)
            del self.connections[ready]
            self.listeners.remove(ready)
            raise WebSocketException('connection failure')
      ...
      for failed in xList:
         if failed == self.serversocket:
            self.close()
            raise WebSocketException('server socket failed')
         else:
