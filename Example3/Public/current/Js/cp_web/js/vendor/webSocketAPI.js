...
)
{
  ...
  return class WebSocketAPI
  {
    ...
    _dispatch( event )
    {
      ...
      if ( echoBack )
      {
        const callback = this.requestHandlers[ echoBack.requestId ]
        if ( callback )
        {
    ...
    requestNotification( name, notificationCallback, requestCallback = null )
    {
      ...
      this.requestHandlers[ notificationId ] =
        function( data )
        {
      ...
      // notification.
      this.requestHandlers[ requestId ] = function( data )
      {
        const isError = data.notificationRequest

        // If there was a failure remove the request handler as it will never
        // receive a notification.
        if ( isError )
          delete self.requestHandlers[ notificationId ]

        if ( requestCallback )
          requestCallback( isError )

        delete self.requestHandlers[ requestId ]
        clearTimeout( self.requestTimeouts[ requestId ] )
        delete self.requestTimeouts[ requestId ]
      }

      this.requestTimeouts[ requestId ] = this._makeTimeout( "Notification request `" + name + "`" )
    ...
    cancelAllNotifications( callback = null, errorCallback = null )
    {
      ...
      const self = this
      this.requestHandlers[ id ] = function( data )
      {
        ...
        delete self.requestHandlers[ id ]
        clearTimeout( self.requestTimeouts[ id ] )
        delete self.requestTimeouts[ id ]
      }

      this.requestTimeouts[ id ] = this._makeTimeout( id )
    ...
    //-------------------------------------------------------------------------
    _requestHandlers( id, deferment, callback )
    {
      const self = this
    ...
    makeRequest( name, parameters = [], callback = null, errorCallback = null )
    {
      ...
      // The temporary request removes handler after data has been received.
      // The inner callback returns just the response data.
      this.requestHandlers[ id ] =
        this._selectRequestHandler( "response", id, callback, deferment, errorCallback )

      this.requestTimeouts[ id ] = this._makeTimeout( "Request `" + name + "`" )
    ...
    makeRequests( requestsData, callback = null )
    {
      ...
      this.requestHandlers[ id ] = this._checkedRequestHandlers( id, callback, deferment )

      this.requestTimeouts[ id ] = this._makeTimeout( "Requests id: " + id )
    ...
    )
    {
      ...
      const self = this
      this.requestHandlers[ notificationName ] =
        function( notificationData )
        {
          ...
          delete self.requestHandlers[ notificationName ]
        }

      // Wrap the callback function.
      // The request handler must be removed after the request has completed.
      const requestId = this._getUniqueId()
      this.requestHandlers[ requestId ] =
      ...
      this.requestTimeouts[ requestId ] = this._makeTimeout( "AsynchronousRequest `" + name + "`" )
    ...
    close()
    {
      ...
      for ( const timer of Object.values( this.requestTimeouts ) )
        clearTimeout( timer )
