...
)
{
  return class System
  {
    ...
    //-------------------------------------------------------------------------
    // Uses:
    //   Load all SVG files for given configuration.
    // Input:
    //   _accutechAPI - Instance of `AccutechAPI`.
    //   callback - Callback to run after loading.
    // Output:
    ...
    _loadAssets( callback )
    {
      ...
      (
        ...
        function( svgData )
        {
          // Store SVG files.
          self.svgFiles = {}
          for ( const [ assetId, svg ] of Object.entries( svgData ) )
            self.svgFiles[ assetId ] = svg
      ...
      for ( const clientView of Object.values( this.clientViews ) )
      {
        ...
        for ( const viewId of viewList )
        {
          if ( ! ( viewId in this.views ) )
          {
            ...
            clientView.views.remove( viewId )
            haveErrorsBeenFound = true
          }
        }
      }
    ...
    //-------------------------------------------------------------------------
    constructor( accutechAPI, clientId, callback )
    {
